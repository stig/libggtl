...
ggtltut2 - a supplement to ggtltut

=head1 SYNOPSIS

This article shows how to create a Tic-Tac-Toe program where we clone
the states instead of providing undo for a particular state.
...
linking against it incurs no extra cost on your memory usage.
...
chose either, but let's pick the C<clone_state()> method:
